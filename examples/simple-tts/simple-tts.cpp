...
int main(int argc, char ** argv) {
    ...
    for (size_t i = 0; i < prompt_inp.size(); ++i) {
        batch_add(batch, prompt_inp[i], i, false);
    }
    ...
    int32_t i_batch = batch.n_tokens - 1;
    ...
    while (n_decode <= n_predict) {
        ...
        if (i_batch < 0) {
            ...
            printf("stream finished: i_batch < 0\n");
            break;
        }

        llama_token new_token_id = llama_sampler_sample(smpl, ctx, i_batch);
        ...
        if (llama_vocab_is_eog(vocab, new_token_id) || n_decode == n_predict) {
            ...
            i_batch = -1;
            printf("stream finished: EOG\n");
            break;
        }

        i_batch = batch.n_tokens;

        batch_add(batch, new_token_id, n_past, true);
    ...
    batch = llama_batch_init(codes.size(), 0, 1);

    printf("codes.size(): %zu\n", codes.size());

    for (size_t i = 0; i < codes.size(); ++i) {
        batch_add(batch, codes[i], i, true); // TODO: all logits?
    }
    ...
    auto audio = embd_to_audio(embd, codes.size(), n_embd, ctx_params.n_threads);
